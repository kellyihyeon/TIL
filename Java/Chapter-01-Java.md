...
1. [자바의 세계](#1-자바의-세계)  
  1.1 [JDK](#11-jdk)   
  1.2 [환경 변수 Path](#12-환경-변수-path)  
  1.3 [실습](#13-실습)

2. [자바 프로그램과 실행의 원리에 대한 이해](#2-자바-프로그램과-실행의-원리에-대한-이해)  
   2.1 [일반적인 프로그램과 자바 프로그램의 차이](#21-일반적인-프로그램과-자바-프로그램의-차이)  
   2.2 [자바 런처](#22-자바-런처)    
   2.3 [자바 프로그램과 자바 가상머신](#23-자바-프로그램과-자바-가상머신)  
   2.4 [자바 컴파일러와 자바 바이트코드](#24-자바-컴파일러와-자바-바이트코드)  
3. [첫 번째 자바 프로그램의 관찰과 응용](#3-첫-번째-자바-프로그램의-관찰과-응용)  
4. [들여쓰기와 컴파일 대상에서 제외되는 주석](#4-들여쓰기와-컴파일-대상에서-제외되는-주석)  
   4.1 [블록 단위 주석](#41-블록-단위-주석)  
   4.2 [행 단위 주석](#42-행-단위-주석)  
   4.3 [들여쓰기](#43-들여쓰기)  
   4.4 [중괄호를 열고 닫는 방식에 대하여](#44-중괄호를-열고-닫는-방식에-대하여)  
<br>
...
# 4. 들여쓰기와 컴파일 대상에서 제외되는 주석
자바 컴파일러가 컴파일을 진행하는 과정은 우리가 책을 읽는 것과 똑같다. (왼쪽에서 오른쪽으로 읽는다.)

  - `/*` 이 표시를 만나면 '지금부터 등장하는 내용은 컴파일 대상에서 제외를 시켜야 되는구나. 그냥 넘어가야 되는구나'라고 해석한다.  
  주석의 끝을 의미하는 `*/` 이 표시를 만날때까지. 

  - 컴파일러는 /* 를 만나면 컴파일은 하지않고 */ 를 찾는다.
  
  - 소스 코드 안에 정보를 남기고 싶다면 주석 안에 필요한 정보를 담아두면 된다.
...
```
- 주석(`/* */`) 사이에 * 가 와도 되는 것일까?  
 (`/*` 묶어서 주석의 시작, `*/` 묶어서 주석의 끝을 나타낸다.)  
  `/*` 은 목표가 `*/` 을 찾는 것이라 * 이런 게 오더라도 계속 `*/` 만 찾아서 상관이 없긴 하지만 좋은 습관은 아니니, 주석 사이에 * 이런 것들은 안넣는 게 좋다.
<br>
...
```
- `//` 을 만나면 이 줄을 넘어가버린다. (한 라인을 주석처리 하게 되는 것)
- // 행 단위 주석을 만나면 더 이상 읽지 않고 다음 행으로 넘어간다.

## 4.3 들여쓰기
들여쓰기는 왜 하는 것일까?  

들여쓰기를 하는 이유는 단순하다. 보기 좋으라고. 

들여쓰기를 안한다면 구분이 힘들어지는데, 들여쓰기를 하면 클래스를 구분하기가 쉬워지고, 메서드가 어디서부터 어디까지인지 구분하기가 쉬워지고, 메서드 안의 내용들이 어디부터 어디까진지 구분하기가 쉬워진다.

들여쓰기는 메모장에서는 수동으로 해야하지만, 개발 툴에서는 자동으로 이뤄진다.

## 4.4 중괄호를 열고 닫는 방식에 대하여
