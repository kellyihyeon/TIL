...
3. [메소드의 재귀 호출](#3-메소드의-재귀-호출)  
   3.1 [수학적 측면에서의 재귀적인 사고](#31-수학적-측면에서의-재귀적인-사고)  
   3.2 [함수식 기반의 메소드 정의](#32-함수식-기반의-메소드-정의)  
   3.3 [메소드의 재귀 호출 흐름](#33-메소드의-재귀-호출-흐름)  
<br>
...
```
- n! = n x (n -1)!  
이 문장을 코드로 그대로 옮기도록 돕는 것이 재귀 메소드의 정의이다.
<br>
...
```
- 자기 자신을 호출해서 돌아간다는 개념으로 생각하지 말고 원본 - 사본을 만들어놓고 다른 팩토리를 호출한다고 생각하면 덜 헷갈린다.
- 수식을 먼저 정의하고 그 수식을 바탕으로 재귀메소드를 정의한다.
<br>


## 3.3 메소드의 재귀 호출 흐름
![flow](./Img/Recursion.png)
