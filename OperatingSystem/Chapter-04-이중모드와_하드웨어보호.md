...
## 2.3. CPU 보호
### 2.3.1 한 사용자가 실수 또는 고의로 CPU 시간 독점
- ```c
    while(n = 1){
      ...
    }
  ```
  위 코드를 보면 n에 1이 들어가는데 C언어에서 이 조건은 항상 true 이기 때문에 루프에서 절대 빠져나오지 못한다.  
  결국 CPU는 User1 의 프로그램만 실행하게 되고 User2, User3 프로그램으로 절대 넘어가지 못한다. 다른 사용자의 프로그램은 실행을 할 수 없다.
  
### 2.3.2 해결법
- Timer를 두어 일정 시간 경과 시 타이머 인터럽트  
  타이머는 CPU에게 일정주기로 신호를 내보낸다.
  (예: 100ms 지나면 매번 인터럽트를 건다.)  
  인터럽트가 들어오면 CPU는 하던 작업을 멈추고 OS의 인터럽트 서비스 루틴으로 점프한다.

- 인터럽트 서비스 루틴안에 어떤 코드를 넣어주면 될까?  
  시간이 골고루 분배되고 있는지, 한 프로그램에게만 가고 있는 건 아닌지 서비스 루틴이 체크할 수 있도록 하는 코드를 넣는다.

- 한 프로그램에 묶여 있다면 인터럽트 서비스 루틴은 CPU 에게 이를 알린다. CPU는 해당 프로그램은 강제종료하고 다음 프로그램으로 넘어갈 수 있도록 한다.